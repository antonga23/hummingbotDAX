...
from hummingbot.connector.exchange.openware.lib.helpers import date_to_milliseconds, interval_to_milliseconds
from hummingbot.connector.exchange.openware.lib.exceptions import OpenwareAPIException, OpenwareRequestException, OpenwareWithdrawException
...
class Client(object):
	...
	def __init__(self, api_key, api_secret, api_url):
		...
		#self.api_url = "https://www.quantaexchange.org/api/v2/peatio"
		#self.api_url = openware_api_url
		print("API URL IS!: ", openware_api_url) # PRINTY
	...
	def update_headers(self):
		...
		session.headers.update({'Accept': 'application/json',
								...
								'X-Auth-Nonce': timestamp,
								'X-Auth-Signature': signature})
		#print("X-Auth-Apikey:", _api_key)
		#print("X-Auth-Nonce:", timestamp)
		#print("X-Auth-Signature:", signature)
		return session

	def _create_api_uri(self, path):
		global openware_api_url
		return "%s%s" % (openware_api_url, path)

	def _generate_signature(self, timestamp):
		global _api_key
		global _api_secret

		query_string = "%s%s" % (timestamp, _api_key)
		m = hmac.new(_api_secret.encode('utf-8'), query_string.encode('utf-8'), hashlib.sha256)
		return m.hexdigest()
	...
	async def get_my_trades(self, **params):
		#print("get_my_trades() ??????")
		result = self._get("/market/trades", data=params)
	...
	async def get_order(self, **params):
		...
		response = self._get("/market/orders", data=params)
		#print("get_order(): ", response)
		return response;
