...
class OpenwareOrderBookTracker(OrderBookTracker):
    ...
    def __init__(self,
                 ...
                 domain: str = "com"):
        super().__init__(
            data_source=OpenwareAPIOrderBookDataSource(trading_pairs=trading_pairs, openware_api_url = openware_api_url, openware_ranger_url = openware_ranger_url, domain=domain),
        ...
        while True:
            try:
                ...
                if message.type is OrderBookMessageType.DIFF:
                    ...
                    now: float = time.time()
                    if int(now / 60.0) > int(last_message_timestamp / 60.0):
                        self.logger().debug("Processed %d order book diffs for %s.", diff_messages_accepted, trading_pair)
