...
#from openware import client as openware_client_module
#from openware.exceptions import OpenwareAPIException
...
from .openware_order_book_tracker import OpenwareOrderBookTracker
from .openware_user_stream_tracker import OpenwareUserStreamTracker
from .openware_in_flight_order import OpenwareInFlightOrder
...
cdef class OpenwareExchangeTransactionTracker(TransactionTracker):
	cdef:
		OpenwareExchange _owner

	def __init__(self, owner: OpenwareExchange):
		super().__init__()
		self._owner = owner
...
cdef class OpenwareExchange(ExchangeBase):
	...
	OPENWARE_TRADE_TOPIC_NAME = "openware-trade.serialized"
	OPENWARE_USER_STREAM_TOPIC_NAME = "openware-user-stream.serialized"
	...
	def __init__(self,
		...
		#self.last_pull_timestamp = 0
		self._trading_required = trading_required
		self._order_book_tracker = OpenwareOrderBookTracker(trading_pairs=trading_pairs, openware_api_url = openware_api_url, openware_ranger_url = openware_ranger_url, domain=domain)
		self._openware_client = OpenwareClient(openware_api_key, openware_api_secret, openware_api_url)
		self._user_stream_tracker = OpenwareUserStreamTracker(openware_client=self._openware_client, domain=domain)
		self._ev_loop = asyncio.get_event_loop()
		self._poll_notifier = asyncio.Event()
		self._last_timestamp = 0
		self._in_flight_orders = {}	 # Dict[client_order_id:str, OpenwareInFlightOrder]
		self._order_not_found_records = {}	# Dict[client_order_id:str, count:int]
		self._tx_tracker = OpenwareExchangeTransactionTracker(self)
	...
	def name(self) -> str:
		if self._domain == "com":
			...
			return f"openware_{self._domain}"
	...
	@property
	def order_book_tracker(self) -> OpenwareOrderBookTracker:
		return self._order_book_tracker

	@property
	def user_stream_tracker(self) -> OpenwareUserStreamTracker:
		...
		async with self._throttler.weighted_task(request_weight=request_weight):
			try:
				...
				if "Timestamp for this request" in str(ex):
					self.logger().warning("Got Openware timestamp error. "
										  "Need to force update Openware server time offset...")
				raise ex
	...
	async def _update_trade_fees(self):
		...
		if current_timestamp - self._last_update_trade_fees_timestamp > 60.0 * 60.0 or len(self._trade_fees) < 1:
			try:
				...
				self.logger().network("Error fetching Openware trade fees.", exc_info=True, app_warning_msg=f"Could not fetch Openware trading fees. Check network connection.")
				raise
				
	cdef object c_get_fee(self, str base_currency, str quote_currency, object order_type, object order_side, object amount, object price):
		cdef:
	...
	def _format_trading_rules(self, exchange_info_dict: Dict[str, Any]) -> List[TradingRule]:
		...
		for rule in trading_pair_rules:
			try:
				...
				retval.append(TradingRule(trading_pair, min_order_size=min_order_size, min_price_increment=Decimal(tick_size), min_base_amount_increment=Decimal(step_size), min_notional_size=Decimal(min_notional)))
			except Exception:
	...
	async def _update_order_fills_from_trades(self):
		...
		cdef:
			...
			int64_t last_tick = <int64_t>(last_pull_timestamp / 10.0)
	...
	async def _history_reconciliation(self):
		...
		cdef:
			...
			int64_t last_tick = <int64_t>(last_poll_timestamp / self.LONG_POLL_INTERVAL)
		...
		if current_tick > last_tick:
			...
			tasks = [self.query_api(self._openware_client.get_my_trades, symbol=convert_to_exchange_trading_pair(trading_pair))
					 for trading_pair in trading_pairs]
					 
					 
			self.logger().debug(f"Polling for order fills of {len(tasks)} trading pairs.")
			print("Polling for order fills of {} trading pairs.".format(len(tasks)))
			
			exchange_history = await safe_gather(*tasks, return_exceptions=True)
			print("exchange_history: ", exchange_history) # PRINTY
	...
	async def _update_order_status(self):
		cdef:
			...
			int64_t last_tick = <int64_t>(self.last_poll_timestamp / self.UPDATE_ORDER_STATUS_MIN_INTERVAL)
		...
		if current_tick > last_tick and len(self._in_flight_orders) > 0:
			...
			for order_update, tracked_order in zip(results, tracked_orders):
				...
				order_type = OpenwareExchange.to_hb_order_type(order_update["type"])
		...
		async for event_message in self._iter_user_event_queue():
			try:
				...
				if event_type == "executionReport":
					...
					print("TRACKED ORDER: ", tracked_order)
	...
	async def _status_polling_loop(self):
		...
		while True:
			try:
				...
				last_poll_timestamp = self._current_timestamp
			except asyncio.CancelledError:
				raise
			except Exception:
	...
	@staticmethod
	def openware_order_type(order_type: OrderType) -> str:
		return order_type.name.upper()

	@staticmethod
	def to_hb_order_type(openware_type: str) -> OrderType:
		return OrderType[openware_type]
	...
	async def execute_buy(self,
		...
		cdef:
			TradingRule trading_rule = self._trading_rules[convert_to_exchange_trading_pair(trading_pair)]
		...
		try:
			...
			if order_type is OrderType.LIMIT or order_type is OrderType.LIMIT_MAKER:
				...
				#print("BUY LIMIT order_decimal_amount:",order_decimal_amount,"order_decimal_price: ", order_decimal_price)
	...
	async def execute_sell(self,
		...
		#print("EXECUTE SELL", "_trading_rules:", self._trading_rules, "trading_pair:", trading_pair)
		cdef:
			TradingRule trading_rule = self._trading_rules[convert_to_exchange_trading_pair(trading_pair)]

		decimal_amount = amount
		# decimal_amount = self.quantize_order_amount(trading_pair, amount)
		decimal_price = (self.c_quantize_order_price(convert_to_exchange_trading_pair(trading_pair), price)
						 if order_type is OrderType.LIMIT or order_type is OrderType.LIMIT_MAKER
						 else s_decimal_0)
		if decimal_amount < trading_rule.min_order_size:
		...
		try:
			...
			if order_type is OrderType.LIMIT or order_type is OrderType.LIMIT_MAKER:
				...
				#print("SELL LIMIT order_decimal_amount:",order_decimal_amount,"order_decimal_price: ", order_decimal_price)
			...
			print("SELL EXECUTE EXCEPTION", str(e))
			self.c_stop_tracking_order(order_id)
	...
	cdef str c_sell(self, str trading_pair, object amount, object order_type=OrderType.MARKET, object price=s_decimal_NaN,
		...
		#print("c_sell()... order_id:", order_id)
	...
	cdef object c_get_order_price_quantum(self, str trading_pair, object price):
		tp = convert_to_exchange_trading_pair(trading_pair)
		cdef:
			TradingRule trading_rule = self._trading_rules[tp]
	...
	# async def get_open_orders(self) -> List[OpenOrder]:
		# orders = await self.query_api(self._openware_client.get_open_orders)
		# ret_val = []
		# for order in orders:
	...
	async def get_all_my_trades(self, trading_pair: str) -> List[Trade]:
		# Ths Openware API call rate is 5, so we cache to make sure we don't go over rate limit
		trades = await self.query_api(self._openware_client.get_my_trades,
									  symbol=convert_to_exchange_trading_pair(trading_pair))
		from hummingbot.connector.exchange.openware.openware_helper import format_trades
	...
	async def get_my_trades(self, trading_pair: str, days_ago: float) -> List[Trade]:
		...
		from hummingbot.connector.exchange.openware.openware_helper import get_utc_timestamp
