...
cdef class PureMarketMakingStrategy(StrategyBase):
	...
	def active_orders(self) -> List[LimitOrder]:
		#print("HELLO active_orders()") # PRINTY
		
		#print("active orders: ", self.market_info_to_active_orders, self._market_info) # PRINTY
		#print("active orders: ", self._market_info) # PRINTY
	...
	cdef c_tick(self, double timestamp):
		...
		try:
			...
			if self._create_timestamp <= self._current_timestamp:
				...
				#print("proposal.buys:", proposal.buys) # PRINTY
				#print("proposal.sells:", proposal.sells)
	...
	cdef object c_create_base_proposal(self):
		...
		if order_override is not None and len(order_override) > 0:
			...
			for level in range(0, self._sell_levels):
				...
				if size > 0:
					sells.append(PriceSize(price, size))
		#print("Proposal - buys: ", buys, " sells: ", sells)
	...
	cdef tuple c_get_adjusted_available_balance(self, list orders):
		...
		cdef:
			ExchangeBase market = self._market_info.market
			object base_balance = market.c_get_available_balance(self.base_asset.lower())
			object quote_balance = market.c_get_available_balance(self.quote_asset.lower())
	...
	cdef c_apply_budget_constraint(self, object proposal):
		...
		#print("base_balance:", base_balance, "quote_balance:", quote_balance)
		for buy in proposal.buys:
	...
	cdef c_execute_orders_proposal(self, object proposal):
		...
		if len(proposal.sells) > 0:
			...
			for sell in proposal.sells:
				...
				#print("SELL!!!!!!!!", sell.price, "ask_order_id:", ask_order_id)
				orders_created = True
		if orders_created:
			self.set_timers()
